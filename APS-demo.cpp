...
public:
  ...
  order(int id, string n, int q, int d, int p, int e)
      : order_id(id), name(n), quantity(q), due_date(d), priority(p),
...
vector<order> generate_random_order(int size = PRODUCT_NUM) {
  ...
  for (int i = 0; i < size; i++) {
    ...
    order o(i, "order" + (i < 10 ? "0" + to_string(i) : to_string(i)),
            rand() % 10000, due_date, rand() % 10, earlest_start_date);
    orders.push_back(o);
  }
  return orders;
}
...
public:
  ...
  machine(int id, string n, int c = 0, int mph = MACHINE_PRODUCT_PER_HOUR)
      : machine_id(id), name(n), finishing_time(c),
...
void printOrders(const vector<order> &orders) {
  for (auto o : orders) {
    cout << "Order: " << o.name << " Quantity: " << fixed << setw(2)
         << o.quantity << " Due Date: " << fixed << setw(2) << o.due_date
         << " Priority: " << fixed << setw(2) << o.priority
         << " Earlest Start Date: " << fixed << setw(2) << o.earlest_start_date
         << endl;
  }
}
...
void printMachines(const vector<machine> &machines) {
  for (auto m : machines) {
    cout << "Machine: " << m.name << " Finishing Time: " << fixed << setw(2)
         << setfill('0') << m.finishing_time
         << " Machine Product Per Hour: " << fixed << setw(2) << setfill('0')
         << m.machine_product_per_hour << " Orders in Queue: \n \t";
...
int main() {
  ...
  priority_queue<machine, vector<machine>, compare_machine_by_free_time>
      machine_queue;
  for (auto m : machines) {
  ...
  while (!machine_queue.empty()) {
    ...
    machine_queue.pop();
    cout << "Machine: " << m.name << " Finishing Time: " << m.finishing_time
         << endl;
  }
  return 0;
}
